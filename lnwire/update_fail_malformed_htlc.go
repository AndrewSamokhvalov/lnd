...
// UpdateFailMalformedHTLC is sent either by payment forwarder or by payment
// receiver to the payment sender in order to notify it that the onion blob
// can't be parsed and for that reason we send this message instead of obfuscate
// the onion failure.
type UpdateFailMalformedHTLC struct {
	...
	// ShaOnionBlob sha of the onion blob on which can't be parsed by the
