...
// Failure represent the onion failure object which is able to identify itself
// by returning the unique failure code.
type Failure interface {
	Code() FailCode
}

// fixedFailureMessageLength is the size of the failure message plus the size
// of padding. Failure message always should be equals to this size.
const fixedFailureMessageLength = 128
...
func (f *FailTemporaryChannelFailure) Decode(r io.Reader, pver uint32) error {
	// Not sure why we need write length here, because the channel
	// update has fixed number of bytes.
	var length uint16
...
func (f *FailAmountBelowMinimum) Decode(r io.Reader, pver uint32) error {
	...
	// Not sure why we need length here, because the channel
	// update has fixed number of bytes, so just read leght, but
	// not use it.
	var length uint16
...
func NewFeeInsufficient(htlcMsat btcutil.Amount, update *ChannelUpdate) *FailFeeInsufficient {
...
func DecodeFailure(r io.Reader, pver uint32) (Failure, error) {
	...
	case Serializable:
		if err := failure.Decode(failureReader, pver); err != nil {
			return nil, err
		}
	}
...
func EncodeFailure(w io.Writer, failure Failure, pver uint32) error {
	...
	if len(failureMessage) > fixedFailureMessageLength {
		return errors.New("failure message exceed max available size")
	}

	code := uint16(failure.Code())
	pad := make([]byte, fixedFailureMessageLength-len(failureMessage))
...
func makeEmptyOnionError(code FailCode) (Failure, error) {
	switch code {
	case CodeInvalidRealm:
