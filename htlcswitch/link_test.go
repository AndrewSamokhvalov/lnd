...
func TestExitNodeTimelockPayloadMismatch(t *testing.T) {
	...
	} else if err.Error() != lnwire.CodeFinalExpiryTooSoon.String() {
...
func TestChannelLinkMultiHopInsufficientPayment(t *testing.T) {
	...
	} else if err.Error() != errors.New(lnwire.CodeInsufficientCapacity).Error() {
...
func TestChannelLinkMultiHopDecodeError(t *testing.T) {
	...
	n.carolChannelLink.cfg.GetHopIterator = func(
		r io.Reader, meta []byte) (HopIterator, error) {
		return nil, sphinx.ErrInvalidOnionVersion
	}
